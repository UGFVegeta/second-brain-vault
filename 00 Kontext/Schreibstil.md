---
tags: [kontext]
---

# Schreibstil und Tonalität

## Grundton

Freundlich, direkt, kurz und verständlich. Kein unnötiger Fülltext. Oskar schreibt so wie er spricht: sachlich, hilfsbereit, auf Augenhöhe.

## Ansprache

Du-Form – bei Schülern, Kollegen und Schulleitung gleichermaßen.

## Regeln

- Kurz und knapp: ein Gedanke, ein Absatz – keine langen Schachtelsätze
- Verständlich: Fachbegriffe nur wenn nötig, sonst einfache Sprache
- Keine Emojis im beruflichen Kontext
- Kein Bürokratie-Deutsch ("Bezugnehmend auf...", "Hiermit teile ich mit...")
- Freundlicher Einstieg, klares Anliegen, klarer Abschluss

## Vermeiden

- Übertriebene Förmlichkeit
- Leere Floskeln ("Ich hoffe, diese Mail findet dich wohlauf")
- Lange Einleitungen bevor das eigentliche Anliegen kommt
- Emojis in Mails und schulischen Texten

## Konkrete Erkenntnisse (aus der Zusammenarbeit)

- **Keine Aufzählungen in Nachrichten/Briefen.** Auch bei mehreren Punkten Fließtext, keine Bulletlisten. Listen wirken zu schematisch.
- **"Zu sehr nach KI" ist ein echtes Ablehnungskriterium.** Vermeiden: glatte, perfekt ausbalancierte Sätze, Dreiklänge/Tricolon, durchgestylte Symmetrie. Lieber plain und gesprochen-natürlich, ruhig mit kurzen, einfachen Sätzen.
- **Gedankenstriche/Bindestriche niemals verwenden.** Oskar nutzt sie nie. Stattdessen Punkt, Komma oder Satz umbauen. (Bestätigt 2026-06-20.)
- **Kein blumiges Lob.** Formulierungen wie "durchweg freundliche und kompetente Beratung, das haben wir sehr geschätzt" sind zu viel und nicht Oskars Stimme. Schlicht bleiben: "vielen Dank für Ihre freundliche Beratung" reicht.
- **Oskars Direktheit nicht glätten.** Pointierte, auch mal provokante Formulierungen ("unterirdisch", kleine Spitzen) bewusst stehen lassen – Abmildern nimmt die Wirkung.
- **Keine relativierenden Zusätze**, die den eigenen Punkt verwässern (Halbsätze wie "das wirkt sich zwar nicht … aus" eher streichen).
- **Wir-Form** statt "unsere Klasse" o. ä. – konsequent "wir".
- **Fakten vor Wirkung:** Lieber eine schwächere, aber korrekte Aussage als eine wirkungsvolle, die sachlich nicht stimmt. Bei Unsicherheit über Regelungen nachfragen statt plausibel klingende Hebel erfinden.
- **"Nur Grammatik und Rechtschreibung" heißt wörtlich:** minimal eingreifen, Wortwahl und Satzbau ansonsten unangetastet lassen.

## Empfänger-spezifischer Ton

- **Konrektor:** Sehr gutes persönliches Verhältnis. Mails an ihn locker und kollegial halten, nicht über-formalisieren. Umgangssprachliches wie „drüberschauen" ist völlig in Ordnung – nur echte Grammatik-/Klarheitsfehler korrigieren, Tonfall entspannt lassen.
- **Schulleitung / Behörden:** Weiterhin formeller und sorgfältiger (siehe auch Selbstkritik-Regel in CLAUDE.md).

## Sprache

Deutsch (primär). Fachbegriffe aus dem Englischen nur wenn es kein gutes deutsches Äquivalent gibt.

## Beispiele für guten Stil

Wird mit der Zeit ergänzt. Hier können Beispieltexte abgelegt werden die Oskars Stil gut repräsentieren – z.B. eine gelungene Elternmail, ein guter Elternbrief, eine klare Kollegennachricht.
