...
function achsen(x0,y0,w,h,xl,yl){
  return '<line x1="'+x0+'" y1="'+y0+'" x2="'+(x0+w+10)+'" y2="'+y0+'" stroke="#9AAAC0" stroke-width="1.5"/>'+
         ...
         G.txt(x0+w+10,y0-10,xl,"end dim")+G.txt(x0+8,y0-h-6,yl,"dim");
}
...
var NUK={F20:["Fluor-20",11,"s"],Rn220:["Radon-220",56,"s"],I131:["Iod-131",8,"Tage"],Cs137:["Cäsium-137",30,"Jahre"],C14:["Kohlenstoff-14",5730,"Jahre"],U238:["Uran-238",4.5e9,"Jahre"]};
function hwz(){
  var d=NUK[S.nuk], T=d[1], k=+S.tn, t=k*T, o=[], x0=100, y0=390, w=660, h=320, i;
  o.push(achsen(x0,y0,w,h,"Zeit in "+d[2],"Anteil in %"));
  ...
  return {svg:o.join(""), readout:'<span class="chip"><b>'+d[0]+'</b>: Halbwertszeit '+(T>=1e6?"4,5 Mrd.":T.toLocaleString("de-DE"))+' '+d[2]+'</span><span class="chip">nach '+tl+' '+d[2]+': <b>'+komma(100*an,1)+' %</b> übrig</span>'};
}
...
function aktiv(){
  ...
  o.push(G.txt(px,280,"Präparat","mid dim")+G.txt(40,410,"Das Zählrohr hat ein Fenster von etwa 1 cm Radius. Es sieht nur einen kleinen Teil der Strahlung.","dim"));
