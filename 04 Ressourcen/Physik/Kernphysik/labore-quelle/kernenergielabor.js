/* Kernenergielabor · Kernphysik Klasse 10 · Leitfragen 5 und 6
   Kernspaltung, Kettenreaktion, Steuerstaebe, Kreisläufe im Kraftwerk, Kernfusion, Abklingen des Abfalls. */
(function(){
"use strict";
...
function staebe(){
  ...
  var p=[]; for(i=0;i<=26;i++){ var v=Math.min(220,100*Math.pow(k,i*4)); p.push(G.f1(x1+i*10)+","+G.f1(y1-v)); }
  o.push('<polyline points="'+p.join(" ")+'" fill="none" stroke="'+(k>1.005?"#FF7A59":(k<0.995?CYAN:GRUEN))+'" stroke-width="3"/>');
  return {svg:o.join(""), readout:'<span class="chip">Steuerstaebe <b>'+s+' %</b> eingefahren</span><span class="chip">Leistung <b>'+P+'</b></span>'};
}
...
function fusion(){
  ...
  o.push(G.txt(60,410,"Sonne: etwa 15 Mio. °C, dafür riesiger Druck und Milliarden Jahre Zeit. Auf der Erde braucht man über 100 Mio. °C.","dim"));
...
window.LAB={state:S,
 ...
 QZ:[
  ...
  {q:"Wozu dienen die Steuerstaebe?", o:["Sie liefern Neutronen","Sie fangen überschüssige Neutronen ein","Sie kühlen den Reaktor"],a:1, w:"Bor oder Cadmium schlucken Neutronen. So bleibt im Mittel genau ein wirksames Neutron pro Spaltung."},
 ...
 scenes:[
  ...
  {kicker:"Schritt 2", title:"Steuerstaebe regeln den Reaktor",
   html:"<p>Steuerstaebe aus Bor oder Cadmium fangen Neutronen ein. Fahre sie hinein oder heraus.</p>",
   ...
   controls:{sl:[{k:"stab",label:"Steuerstaebe eingefahren",min:0,max:100,step:5,fmt:function(v){return v+" %"}}],reset:{stab:50}},
   cfg:{mode:"stab"}, alt:"Reaktorkern mit Brennstaeben und Steuerstaeben"},
