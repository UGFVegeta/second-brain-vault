---
tags: [ressource, physik, unterricht, klasse10, kernphysik, interaktiv]
status: aktiv
date: 2026-09-25
---

# Kernphysik Klasse 10: Stunden und Labore

Alle Einzelstunden des Kernphysik-Blocks W01 bis W22 nach dem [[Stoffverteilungsplan Physik Klasse 10 2026-27]]. Gleicher Aufbau wie die Optik-Stunden in Klasse 7: Tabs Überblick, Folien, Hintergrund, Arbeitsblätter, Folien im Labor-Stil B. W10/W11 (Klassenarbeit und Besprechung) und W14 (Puffer) haben keine eigene Stunde.

| Woche | Stunde | Blatt | Labor |
|---|---|---|---|
| W01 | [[Kernphysik – W01 Woraus besteht Materie – Stunde.html\|Woraus besteht Materie?]] | W01 Atome enthalten elektrische Ladungen (Versuch) | Atomlabor |
| W02 | [[Kernphysik – W02 Atombau und Isotope – Stunde.html\|Atombau und Isotope]] | W02 Nuklide und Isotope, Wiederholung Atombau bei Bedarf | Atomlabor |
| W03 | [[Kernphysik – W03 Zaehlrohr und Nullrate – Stunde.html\|Zählrohr und Nullrate]] | W03 Zählrohr und Nullrate (Oskars Lückentext) | Strahlungslabor |
| W04 | [[Kernphysik – W04 Alpha Beta Gamma – Stunde.html\|Alpha, Beta und Gamma]] | W04 Drei Strahlungsarten | Strahlungslabor |
| W05 | [[Kernphysik – W05 Zerfallsgleichungen – Stunde.html\|Zerfallsgleichungen und Durchdringung]] | W05 Durchdringung und Zerfallsgleichungen | Strahlungslabor |
| W06 | [[Kernphysik – W06 Wuerfelmodell – Stunde.html\|Der Zufall beim Zerfall]] | W06 Zerfall mit Würfeln (Versuch) | Zerfallslabor |
| W07 | [[Kernphysik – W07 Halbwertszeit – Stunde.html\|Die Halbwertszeit]] | W07 Halbwertszeit mit Bierschaum-Versuch | Zerfallslabor |
| W08 | [[Kernphysik – W08 Aktivitaet – Stunde.html\|Aktivität und Zählrate]] | W08 Aktivität und Zählrate | Zerfallslabor |
| W09 | [[Kernphysik – W09 Uebungen Halbwertszeit – Stunde.html\|Übungen zur Halbwertszeit]] | W09 Übungsblatt (Vorbereitung Klassenarbeit) | Zerfallslabor |
| W12 | [[Kernphysik – W12 Ionisierende Strahlung – Stunde.html\|Ionisierende Strahlung]] | W12 Ionisierende Strahlung | Wirkungslabor |
| W13 | [[Kernphysik – W13 Wirkung auf den Koerper – Stunde.html\|Wirkung auf den Körper]] | W13 Wirkung auf den Körper | Wirkungslabor |
| W15 | [[Kernphysik – W15 Schutz und Anwendungen – Stunde.html\|Schutz und Anwendungen]] | W15 Schutz und Anwendungen | Wirkungslabor |
| W16 | [[Kernphysik – W16 Kernspaltung – Stunde.html\|Die Kernspaltung]] | W16 Kernspaltung | Kernenergielabor |
| W17 | [[Kernphysik – W17 Kettenreaktion – Stunde.html\|Kettenreaktion und Reaktor]] | W17 Dominoversuch | Kernenergielabor |
| W18 | [[Kernphysik – W18 Kraftwerk und Fusion – Stunde.html\|Kernkraftwerk und Kernfusion]] | W18 Kernkraftwerk und Kernfusion | Kernenergielabor |
| W19 | [[Kernphysik – W19 Uebungen Kernenergie – Stunde.html\|Leitfrage 5 abschließen]] | W19 Übungen, Reaktorunfälle | Kernenergielabor |
| W20 | [[Kernphysik – W20 Radioaktiver Abfall – Stunde.html\|Wohin mit dem Abfall?]] | W20 Abfall, Endlager, Rückbau | Kernenergielabor |
| W21 | [[Kernphysik – W21 Argumente abwaegen – Stunde.html\|Nutzen und Risiko abwägen]] | W21 Fakt oder Meinung, Diskussion mit Rollen | Kernenergielabor |
| W22 | [[Kernphysik – W22 Rueckblick und C-14 – Stunde.html\|Rückblick und Altersbestimmung]] | W22 Altersbestimmung mit C-14 | Zerfallslabor |

## Aus dem Buch eingearbeitet

Ideen aus „Erlebnis Physik“ Kl. 10, in eigenen Worten.

- **W01 bis W05** (eingearbeitet am 25.09.2026):
  - Ladungsversuche mit Luftballon und Wasserstrahl, Zeitstrahl zur Atomgeschichte
  - Kurzschreibweise und A = Z + N
  - Luftballon-Radioaktivität
  - Ablenkung im elektrischen Feld und Reichweiten
  - Bleidicke 13 mm für 50 %, Zerfallsreihe U-238, Strahlung als Werkzeug
- **W06 bis W22** (ebenfalls am 25.09.2026):
  - Bierschaum als exponentielle Abnahme
  - Exponentielle Abnahme im Alltag
  - Aufgaben zur Halbwertszeit
  - C-14- und Tritium-Methode, Roter Franz
  - Früh-, Spät- und genetische Schäden, fünf A-Regeln, Radon
  - Hahn, Straßmann und Meitner, Dominoversuch zur Kettenreaktion
  - Drei Wasserkreisläufe, Fusion und Wendelstein 7-X
  - Tschernobyl und Fukushima, Rückbau (kontaminiert und aktiviert), Wirtsgesteine
  - Lerncheck-Fragen als Übungen und Kurz-Checks
- Alle Aufgaben auf den Blättern tragen Schwierigkeitskreise.

## Wo vom Buch abgewichen wurde

- **β-Abschirmung:** 5 mm Aluminium statt 1 mm, das passt zum Schulpräparat Sr-90/Y-90.
- **Strahlenbelastung:** Werte nach BfS statt der Buchwerte.
  - natürlich 2,1 mSv
  - Medizin etwa 1,5 mSv
  - Flug nach New York und zurück etwa 0,1 mSv
  - Röntgen Brustkorb 0,01 bis 0,03 mSv
  - CT Kopf etwa 2 mSv
- **Folie 39/40:** „etwa ebenso viel aus der Medizin“ ist ersetzt durch „etwa 1,5 mSv“.
- **Schacht Konrad:** Laut BGE wird er erst bis Ende 2029 fertig, die Einlagerung beginnt Anfang der 2030er. Das Buch schreibt, dort werde schon eingelagert.

## Labore

Startseite für IServ: `Kernphysik-Labore.html` (ein Link, fünf Labore).

- **Atomlabor**: Zoom vom Atom zum Kern, Rutherford, Atom bauen, Isotope, Nuklidkarte.
- **Strahlungslabor**: Nullrate, Luftballon, Präparat und Abstand, α/β/γ, elektrisches Feld, Zerfallsgleichungen, Absorber, Bleidicke.
- **Zerfallslabor**: 100 Würfel, 400 Kerne mit Zufallszerfall, Halbwertszeit für sechs Nuklide, Aktivität und Zählrate, Alltag (Luftdruck, Koffein, Bierschaum), C-14-Uhr.
- **Wirkungslabor**: Ionisation durch α/β/γ, Nebelkammer, Treffer in der Zelle, persönliche Jahresdosis, Schutzregeln mit Reglern.
- **Kernenergielabor**: Spaltung mit drei Bruchstückpaaren, Kettenreaktion, Steuerstäbe, drei Wasserkreisläufe, Fusion mit Temperatur, Abklingen des Abfalls.

## Technik

- **Stunden:**
  - `baue_stunden_k10.py` für W01 bis W05
  - `baue_stunden_k10_teil2.py` für W06 bis W22
  - beide nutzen `../Optik/stunde_vorlage.py`
- **Folien-Zeichnungen:** `kern_zeichnungen.py` (Folien 2 bis 20) und `kern_zeichnungen2.py` (Folien 24 bis 71).
- **Blätter:** `Materialien/baue_blaetter_k10.py` und `Materialien/baue_blaetter_k10_teil2.py`.
- **Labore:** `baue_labore_k10.py` baut alle fünf Labore und die Startseite, die Quellen liegen in `labore-quelle/`.
- **Achtung:** `Kernphysik.pdf` ist noch der alte Stand vom August. `Folien bauen.command` macht aus *jeder* HTML eine PDF, also auch aus Stunden und Laboren.
- **Präparate:** nur durch die Lehrkraft (RiSU).
