"""Gemeinsame Materialliste für die Stunden eines Themas: Gesamtliste und nach Stunden, Status pro Ding
(vorhanden, nicht vorhanden, bestellen, bestellt) im Browser anklickbar. Genutzt von Kernphysik/baue_material_k10.py
und Optik/baue_material_optik.py.
Eine Stunde: {"id": eindeutig, "marke": "W06" oder "F2", "titel", "datum": Anzeigetext, "datei": Stundendatei oder "",
              "zeilen": [{"name", "anz", "hinweis", "rolle": "demo"|"gruppe"}], "hinweis": Hinweis der Stunde}"""
import json
from pathlib import Path


def baue_materialliste(ziel, titel, h1, untertitel, stunden, art, vorgabe, key, h1kurz=None):
    """art: [(Gruppe, [Namen])], die letzte Gruppe fängt alles Übrige; vorgabe: {Name: {"status": v|n|b|o, "notiz": ...}}"""
    art_von = {n: a for a, ns in art for n in ns}
    rest = art[-1][0]
    dinge = {}
    for s in stunden:
        for z in s["zeilen"]:
            d = dinge.setdefault(z["name"], {"name": z["name"], "art": z.get("art") or art_von.get(z["name"], rest), "einsatz": []})
            d["einsatz"].append({"w": s["id"], "anz": z["anz"], "rolle": z["rolle"], "hinweis": z["hinweis"]})
    reihenfolge = {n: j for _, ns in art for j, n in enumerate(ns)}
    gruppen = [a for a, _ in art]
    gesamt = sorted(dinge.values(), key=lambda d: (gruppen.index(d["art"]), reihenfolge.get(d["name"], 99), d["name"].lower()))
    daten = json.dumps({"stunden": stunden, "dinge": gesamt, "arten": gruppen, "vorgabe": vorgabe}, ensure_ascii=False)
    seite = (SEITE.replace("__TITEL__", titel).replace("__H1KURZ__", h1kurz or h1).replace("__H1__", h1)
             .replace("__SUB__", untertitel).replace("__KEY__", key).replace("__DATEN__", daten))
    Path(ziel).write_text(seite, encoding="utf-8")
    print("geschrieben:", Path(ziel).name, "·", len(gesamt), "Dinge aus", sum(1 for s in stunden if s["zeilen"]), "Stunden")
    ohne = [d["name"] for d in gesamt if d["art"] == rest]
    if ohne:
        print(f"unter „{rest}“:", ohne)
...
<header>
<h1>__H1__</h1>
<p class="sub">__SUB__ Status anklicken, er bleibt in diesem Browser gespeichert. Bei „nicht vorhanden“ steht der Ersatz aus der Stunde dabei.</p>
...
const stunde=w=>D.stunden.find(s=>s.id===w);
const link=w=>stunde(w).datei?`<a class="w" href="${encodeURI(stunde(w).datei)}" target="_blank" title="${esc(stunde(w).titel)}">${esc(stunde(w).marke)}</a>`:`<span class="w">${esc(stunde(w).marke)}</span>`;
function bedarf(e){
...
function gesamt(){
  ...
  for(const art of D.arten){
    ...
    for(const d of xs){
      ...
      const ers=zust(d.name).status==="n"?[...new Set(d.einsatz.map(e=>stunde(e.w)).filter(s=>/^Ohne/.test(s.hinweis)).map(s=>s.marke+": "+s.hinweis))]:[];
...
function stunden(){
  ...
  for(const s of D.stunden){
    ...
    h+=`<div class="karte"><h3>${link(s.id)}${esc(s.titel)}<span class="dat">${esc(s.datum)}</span></h3>`;
    if(!s.zeilen.length) h+=s.offen?'<div class="nichts">Noch nicht vorbereitet, das Material kommt später dazu.</div>':'<div class="nichts">Kein Material, nur Folien und Tafel.</div>';
    else{
...
document.getElementById("kopieren").onclick=async()=>{
  ...
  const txt=xs.length?"Bestellen für __H1KURZ__:\n"+xs.map(d=>"- "+d.name+" ("+d.einsatz.map(e=>stunde(e.w).marke+": "+e.anz.replace("×","")+(e.rolle==="gruppe"?(gruppen&&/\d/.test(e.anz)?" pro Gruppe, "+parseInt(e.anz.match(/\d+/)[0])*gruppen+" gesamt":" pro Gruppe"):"")).join("; ")+")"+(zust(d.name).notiz?" – "+zust(d.name).notiz:"")).join("\n"):"Nichts auf „bestellen“ gesetzt.";
