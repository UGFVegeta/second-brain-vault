...
def bogengerade(von, bis, start, delta, schritt=1, kpe=3):
    ...
    s.append(f'<path d="M{x1:.1f} {ax - 5} Q{xm:.1f} {ay:.1f} {x2:.1f} {ax - 5}" fill="none" '
              f'stroke="{farbe}" stroke-width="2.2" marker-end="url(#ar{id(start)}{id(delta)})"/>')
    s.append(f'<defs><marker id="ar{id(start)}{id(delta)}" markerWidth="9" markerHeight="9" refX="6" refY="4" '
...
zeilen_mi = "".join([
    grp(0, "Start", 0, 4),
    vz(4, "Ankommen. Kurzer Rückblick: gestern Zahlengerade und Vergleichen, heute geht es auf der Geraden weiter, "
           "jetzt wird gerechnet", "&ndash;", "Plenum"),
    grp(1, "Einstieg: Bogen-Modell", 4, 12),
    vz(4, f"Erste Rechnung an die Tafel: (−2) + (+7). Frage: Wo stehen wir, wohin bewegen wir uns?",
       "Tafel", "Plenum"),
    vz(4, "Zweite Rechnung: (−1) + (−6). Vergleich der beiden Bögen: positiv addieren = nach rechts, "
          "negativ addieren = nach links", "Tafel", "Plenum"),
    grp(2, "Merksatz", 12, 20),
    vz(8, "Regel in zwei Fällen: gleiche Vorzeichen (Beträge addieren, Vorzeichen bleibt) und verschiedene "
          "Vorzeichen (Beträge subtrahieren, Vorzeichen der betragsmäßig größeren Zahl). Mit den vier "
          "Buch-Beispielen, dann Schreibweise vereinfachen", f"Tafel &rarr; {MK}", "Plenum, abschreiben", True),
    grp(3, "Viel rechnen", 20, 40),
    vz(4, "Aufgabe 1: fehlenden Wert an der Zahlengeraden ergänzen", f"S. 18 Nr. 1 {ALL if False else ''}"
          .replace("{ALL if False else ''}", ''), "Einzel"),
    vz("", "&ndash;", "&ndash;", "&ndash;"),
])

# Hinweis: obiger Platzhalter-Fehler wird unten korrekt ersetzt.
